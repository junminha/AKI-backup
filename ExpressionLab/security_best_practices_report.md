...
> **주의 (2026-09-09 갱신):** 이 보고서는 LLM 공급자가 Groq이던 시점의 점검 결과입니다.
> 이후 OpenRouter(`https://openrouter.ai/api/v1`)로 교체했고, 호출 한도·rate limit·응답
> 캐시를 제거했습니다. 본문의 "Groq"는 "OpenRouter"로, `server.js` 줄 번호는 현재 코드와
> 다르게 읽어 주세요. 아래 결론(키는 서버 `.env`에서만 읽음, LLM 응답 정제, 외부 요청은
> 하드코딩된 단일 엔드포인트)은 그대로 유효합니다.
