...
const ATTENTION_DIRECTIONS: Partial<Record<BoneName, [number, number, number]>> = {
...
export class PoseSolver {
  ...
  apply(frame: TrackFrame, dt: number) {
    ...
    for (const [boneName, pair] of Object.entries(LIMB_CHAIN) as [
      ...
      if (!a || !b || conf < 0.4) {
        const attention = ATTENTION_DIRECTIONS[boneName];
        if (attention) this.aimBone(boneName, new THREE.Vector3(...attention), alpha);
  ...
  private applyAttentionArms(alpha: number) {
    for (const name of [
      ...
      const direction = ATTENTION_DIRECTIONS[name];
      if (direction) this.aimBone(name, new THREE.Vector3(...direction), alpha);
    }
  }
