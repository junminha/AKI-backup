...
export class Tracker {
  ...
  private loop = () => {
    ...
    if (this.dirty) {
      if (!this.building) void this.ensureModels();
      return;
    }
    ...
    try {
      ...
      // A MediaPipe task can become invalid after a GPU/context interruption.
      // Rebuild it before processing another frame instead of repeating Aborted().
      for (const task of [this.pose, this.face, this.hand]) {
        try {
          task?.close();
        } catch {
          // The failed task may already be closed internally.
        }
      }
      this.pose = this.face = this.hand = null;
      this.delegate = "CPU";
      this.dirty = true;
      this.cb.onStatus?.("추론 엔진 복구 중…");
      return;
    }
